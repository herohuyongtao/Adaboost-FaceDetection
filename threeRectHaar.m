...
%                 count = count+1;
%                 feat = zeros(H,W); %initialize to 0 
%                 feat(x:x+h,y:y+w)=1; %white 1
%                 feat(x:x+h,y+w:y+2*w)=-1; % center black
%                 feat(x:x+h,y+2*w:y+3*w-1)=1; % white 2
%                 haarCFvec(:,count) =feat(:); %vectorize feature 
%             end
%         end
%     end
...
%this one gets everything except for the last column
%only problem i think i see is that it's never a 3x1 smallest is 3x2